...
workflow GATK_VARIANTCALL {
    ...
    // The meta gains a `num_chroms` field so the downstream groupTuple can
    // reconstruct a `groupKey(meta, N)` from process outputs. (groupKey itself
    // does not survive Nextflow process I/O — the meta map does.)
    //
    ch_hc_sample = ch_bam
        .join(ch_bai,           by: 0)
    ...
    //   Re-wrap the meta as `groupKey(meta, num_chroms)` so groupTuple emits
    //   exactly once per sample (when all N per-chrom outputs have arrived).
    //   The extra `num_chroms` field in meta is harmless downstream — meta is
    //   used by tag "${meta.id}" / publishDir paths, both of which still work.
    //
