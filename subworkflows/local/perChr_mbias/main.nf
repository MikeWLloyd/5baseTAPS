...
workflow PERCHROMOSOME_MBIAS {
    ...
    // Fan out: one SLURM job per (sample, chromosome).
    // groupKey(meta, n) tells groupTuple how many items to expect so it waits
    // for ALL chromosomes instead of emitting as soon as the channel drains.
    ...
    // Extract fasta path as a single-item value channel so .combine() broadcasts
    // to all samples rather than producing a cross-product
